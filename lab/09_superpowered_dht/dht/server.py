...
def find(key):
    ...
    return f"Node responsible for k={key} is {n.name}"
...
def predecessor():
    if request.method == 'GET':
        ...
        newpred = Process(data[0], int(data[1]), data[2])
...
def join():
    data = request.json
    joiner_id = data.id
    info(f"{app.proc.hostportname()} is trying to welcome {joiner_id}")

    joiner = Process(data.host, data.port, joiner_id)
    info(f"This is what we know about the joiner:\n{joiner.toJSON()}")
    responsible, _ = find_node(app.proc, joiner_id)
    info(f"This is what we know about the responsible:\n{responsible.toJSON()}")

    pred_of_responsible = responsible.pred
    succ_of_responsible = responsible.succ
    info(f"This is what we know about the pred_of_responsible:\n{pred_of_responsible.toJSON()}")

    predurl = f"http://{joiner.host}:{joiner.port}/pred"
    succurl = f"http://{joiner.host}:{joiner.port}/succ"

    requests.put(predurl, data=pred_of_responsible.toJSON())
    requests.put(succurl, data=responsible.toJSON())
